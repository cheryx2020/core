...
const ListArticle = ({data = [], isMobile, Image, Link}) => {
  ...
  return <div className={styles.wrapper}>
    {!isMobile && bigItem && <Link href={`/tip/${bigItem.id}`}><div className={styles.left}>
      <a href={`/tip/${bigItem.id}`} className={styles.bigImage}>
        {bigItem && <Image layout='fill' alt={bigItem.title} src={transformImageSrc(bigItem.imgUrl)}/>}
      </a>
        ...
        <Link href={`/tip/${item.id}`}>
          <a className={styles.item}>
            <div className={styles.img}>
              <Image layout='fill' alt={item.title} src={transformImageSrc(item.imgUrl)}/>
            </div>
