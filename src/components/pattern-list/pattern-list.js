...
const PatternList = ({ useRouter = () => {}, useDispatch = () => {}, data, isAdmin, style = {}, isBottom, className }) => {
return <div className={`${styles.wrapper} ${isBottom ? `${styles.bottom} ${className}` : ''}`} style={{ ...style }}>
        {Array.isArray(data) && data.map((item, index) => <PatternItem useDispatch={useDispatch} useRouter={useRouter} isBottom={isBottom} isAdmin={isAdmin} key={index} {...item} />)}
