...
const AdminMenu = ({isEdit, text, onSaveClick = () => {}, onEditClick = () => {}, onCancelClick = () => {}, onPreviewClick = () => {}, hideButtons = [] }) => {
    ...
    return <div onClick={() => { setIsShowMenu(!isShowMenu) }} className={`${styles.bigMenu}${isShowMenu ? ' ' + styles.menuActive : ''}`}>
        <span>{text}</span>
        {!hideButtons.includes('edit-save') && <div onClick={(e) => { e.stopPropagation(); setIsShowMenu(!isShowMenu); _isEdit ? onSaveClick() : onEditClick(); }} className={`${styles.btn}${isShowMenu ? ` ${styles.top} ` + styles.show : ''}`}>{_isEdit ? 'Save' : 'Edit'}</div>}
