...
const Footer = ({ isMobile, theme, socialTitle = "Tài khoản cá nhân", image = "https://gocnhacolen.com/images/footer.webp", hasBanner, className, isAdmin }) => {
      ...
      <div className={`${styles.banner} ${isShowBanner ? styles.show : styles.hide}`}>
        <div className={styles.bannerImgWrapper}>
          ...
          {linkBanner && <a onClick={() => {
            setIsShowBanner(false);
            localStorage.setItem('closedBannerAt', new Date().getTime());
          }} target='_blank' href={linkBanner}><img alt={process?.env?.NEXT_PUBLIC_SEO_mainTitle} src={imageBanner} /></a>}
        </div>
      </div> : null}
